package com.ceev.wcee.pay.wx.api;
...
import com.ceev.wcee.pay.bean.BillType;
import com.ceev.wcee.pay.bean.PayOrder;
import com.ceev.wcee.pay.http.HttpStringEntity;
import com.ceev.wcee.pay.util.Util;
import com.ceev.wcee.pay.api.BasePayService;
import com.ceev.wcee.pay.bean.MethodType;
import com.ceev.wcee.pay.bean.Order;
import com.ceev.wcee.pay.bean.PayMessage;
import com.ceev.wcee.pay.bean.PayOutMessage;
import com.ceev.wcee.pay.bean.RefundOrder;
import com.ceev.wcee.pay.bean.SignType;
import com.ceev.wcee.pay.bean.TransactionType;
import com.ceev.wcee.pay.bean.TransferOrder;
import com.ceev.wcee.pay.bean.TransferType;
import com.ceev.wcee.pay.bean.result.PayException;
import com.ceev.wcee.pay.exception.PayErrorException;
import com.ceev.wcee.pay.http.ClientHttpRequest;
import com.ceev.wcee.pay.http.HttpConfigStorage;
import com.ceev.wcee.pay.http.UriVariables;
import com.ceev.wcee.pay.util.DateUtils;
import com.ceev.wcee.pay.util.XML;
import com.ceev.wcee.pay.util.sign.SignUtils;
import com.ceev.wcee.pay.util.sign.encrypt.RSA2;
import com.ceev.wcee.pay.util.str.StringUtils;
import com.ceev.wcee.pay.wx.bean.RedpackOrder;
import com.ceev.wcee.pay.wx.bean.WxPayBillType;
import com.ceev.wcee.pay.wx.bean.WxPayError;
import com.ceev.wcee.pay.wx.bean.WxPayMessage;
import com.ceev.wcee.pay.wx.bean.WxRefundResult;
import com.ceev.wcee.pay.wx.bean.WxSendredpackType;
import com.ceev.wcee.pay.wx.bean.WxTransactionType;
import com.ceev.wcee.pay.wx.bean.WxTransferType;
...
public class WxPayService extends BasePayService<WxPayConfigStorage> implements WxRedPackService, WxBillService {
    ...
    public BasePayService setPayConfigStorage(WxPayConfigStorage payConfigStorage) {
        ...
        if (WxConst.HMAC_SHA256.equals(signType)) {
            payConfigStorage.setSignType(WxConst.HMACSHA256);
        }
    ...
    public String getReqUrl(TransactionType transactionType) {
        ...
        return WxConst.URI + (payConfigStorage.isTest() ? WxConst.SANDBOXNEW : "") + transactionType.getMethod();
    }
    ...
    public boolean verify(Map<String, Object> params) {
        ...
        if (null == params.get(WxConst.SIGN) || !(WxConst.SUCCESS.equals(params.get(WxConst.RETURN_CODE)) && WxConst.SUCCESS.equals(params.get(WxConst.RESULT_CODE)))) {
            if (LOG.isErrorEnabled()) {
                LOG.error(String.format("微信支付异常：return_code=%s,参数集=%s", params.get(WxConst.RETURN_CODE), params));
            }
            return false;
        }

        try {
            return signVerify(params, (String) params.get(WxConst.SIGN));
        }
    ...
    private boolean signVerify(Map<String, Object> params, String sign, boolean isTest) {
        ...
        String content = SignUtils.parameterText(params, "&", WxConst.SIGN, "appId") + "&key=" + (signUtils == SignUtils.MD5 ? "" : keyPrivate);
    ...
    private Map<String, Object> getPublicParameters() {
        ...
        parameters.put(WxConst.NONCE_STR, SignUtils.randomStr());
        return parameters;
...
/*        setParameters(parameters, "notify_url", order);
        ...
        if (!WxConst.SUCCESS.equals(result.get(WxConst.RETURN_CODE)) || !WxConst.SUCCESS.equals(result.get(WxConst.RESULT_CODE))) {
            throw new PayErrorException(new WxPayError(result.getString(WxConst.RESULT_CODE), result.getString(WxConst.RETURN_MSG_CODE), result.toJSONString()));
        }
        return result;
    }
    ...
    public Map<String, Object> orderInfo(PayOrder order) {
        ...
        if (verify(preOrderHandler(result, order))) {
            ...
            if (WxTransactionType.JSAPI == order.getTransactionType()) {
                ...
                params.put("nonceStr", result.get(WxConst.NONCE_STR));
                ...
                params.put(WxConst.APPID, payConfigStorage.getAppId());
            ...
            params.put(WxTransactionType.JSAPI.equals(order.getTransactionType()) ? "paySign" : WxConst.SIGN, paySign);
            return params;
        }
        throw new PayErrorException(new WxPayError(result.getString(WxConst.RETURN_CODE), result.getString(WxConst.RETURN_MSG_CODE), "Invalid sign value"));
    ...
    private Map<String, Object> setSign(Map<String, Object> parameters) {
        ...
        if (WxConst.HMACSHA256.equals(signTypeStr)) {
            signTypeStr = SignUtils.HMACSHA256.getName();
        }
        parameters.put("sign_type", signTypeStr);
        String sign = createSign(SignUtils.parameterText(parameters, "&", WxConst.SIGN, "appId"), payConfigStorage.getInputCharset());
        parameters.put(WxConst.SIGN, sign);
        return parameters;
    }
    ...
    private String getKeyPrivate() {
        ...
        String sign = createSign(SignUtils.parameterText(parameters, "&", WxConst.SIGN, "appId"), payConfigStorage.getInputCharset(), false);
        parameters.put(WxConst.SIGN, sign);
        ...
        LOG.error("获取sandbox_signkey失败", new PayErrorException(new PayException(result.getString(WxConst.RETURN_CODE), result.getString(WxConst.RETURN_MSG_CODE), result.toJSONString())));
        return null;
    }
    ...
    public String buildRequest(Map<String, Object> orderInfo, MethodType method) {
        if (!WxConst.SUCCESS.equals(orderInfo.get(WxConst.RETURN_CODE))) {
            throw new PayErrorException(new WxPayError((String) orderInfo.get(WxConst.RETURN_CODE), (String) orderInfo.get(WxConst.RETURN_MSG_CODE)));
        }
    ...
    public WxRefundResult refund(RefundOrder refundOrder) {
        ...
        setParameters(parameters, WxConst.OUT_TRADE_NO, refundOrder.getOutTradeNo());
    ...
    public Map<String, Object> downloadbill(Date billDate, String billType) {
        ...
        ret.put(WxConst.RETURN_CODE, WxConst.SUCCESS);
        ret.put(WxConst.RETURN_MSG_CODE, "ok");
    ...
    public Map<String, Object> downloadbill(Date billDate, String billType, String path) {
        ...
        try {
            ...
            e.printStackTrace();
            Map<String, Object> ret = new HashMap<String, Object>(3);
            ret.put(WxConst.RETURN_CODE, WxConst.FAIL);
            ret.put(WxConst.RETURN_MSG_CODE, "fail");
    ...
    public Map<String, Object> secondaryInterface(Object transactionIdOrBillDate, String outTradeNoBillType, TransactionType transactionType) {
        ...
        if (transactionType == WxTransactionType.DOWNLOADBILL) {
            ...
            throw new PayErrorException(new PayException(WxConst.FAILURE, "非法类型异常:" + transactionIdOrBillDate.getClass()));
        }
        ...
        //获取公共参数
        Map<String, Object> parameters = getPublicParameters();
        setParameters(parameters, WxConst.OUT_TRADE_NO, outTradeNoBillType);
    ...
    public Map<String, Object> transfer(TransferOrder order) {
        ...
        if (null != order.getTransferType() && WxTransferType.TRANSFERS == order.getTransferType()) {
            ...
            parameters.put(WxConst.MCH_ID, payConfigStorage.getPid());
            order.setTransferType(WxTransferType.PAY_BANK);
            payBank(parameters, order);
        }
        parameters.put(WxConst.SIGN, createSign(SignUtils.parameterText(parameters, "&", WxConst.SIGN), payConfigStorage.getInputCharset()));
    ...
    public Map<String, Object> transferQuery(String outNo, String wxTransferType) {
        ...
        if (WxTransferType.TRANSFERS.getType().equals(wxTransferType) || WxTransferType.GETTRANSFERINFO.getType().equals(wxTransferType)) {
             parameters.put(WxConst.APPID, payConfigStorage.getAppId());
             parameters.put(WxConst.SIGN, createSign(SignUtils.parameterText(parameters, "&", WxConst.SIGN), payConfigStorage.getInputCharset()));
            return getHttpRequestTemplate().postForObject(getReqUrl(WxTransferType.GETTRANSFERINFO), XML.getMap2Xml(parameters), JSONObject.class);
        }
        parameters.put(WxConst.SIGN, createSign(SignUtils.parameterText(parameters, "&", WxConst.SIGN), payConfigStorage.getInputCharset()));
        //默认查询银行卡的记录
        return getHttpRequestTemplate().postForObject(getReqUrl(WxTransferType.QUERY_BANK), XML.getMap2Xml(parameters), JSONObject.class);
    }


    public String keyPublic(String content) {
        try {
            return RSA2.encrypt(content, payConfigStorage.getKeyPublic(), WxConst.CIPHER_ALGORITHM, payConfigStorage.getInputCharset());
        }
    ...
    public Map<String, Object> sendredpack(RedpackOrder redpackOrder) {
        ...
        if (WxSendredpackType.SENDMINIPROGRAMHB != transferType || WxConst.FAIL.equals(resp.getString(WxConst.RESULT_CODE))) {
            return resp;
        }
        Map<String, Object> params = new TreeMap<String, Object>();
        params.put("appId", payConfigStorage.getAppId());
        params.put("timeStamp", System.currentTimeMillis() / 1000 + "");
        params.put("nonceStr", parameters.get(WxConst.NONCE_STR));
