package com.ceev.wcee.pay.ali.api;
...
import com.ceev.wcee.pay.ali.bean.AliPayBillType;
import com.ceev.wcee.pay.ali.bean.AliPayConst;
import com.ceev.wcee.pay.ali.bean.AliPayMessage;
import com.ceev.wcee.pay.ali.bean.AliRefundResult;
import com.ceev.wcee.pay.ali.bean.AliTransactionType;
import com.ceev.wcee.pay.ali.bean.AliTransferType;
import com.ceev.wcee.pay.ali.bean.CertEnvironment;
import com.ceev.wcee.pay.ali.bean.OrderSettle;
...
public class AliPayService extends BasePayService<AliPayConfigStorage> {
    ...
    public String getReqUrl(TransactionType transactionType) {
        return payConfigStorage.isTest() ? AliPayConst.DEV_REQ_URL : AliPayConst.HTTPS_REQ_URL;
    }
    ...
    public boolean verify(Map<String, Object> params) {
        ...
        if (params.get(AliPayConst.SIGN) == null) {
            LOG.debug("支付宝支付异常：params：" + params);
            return false;
        }

        return signVerify(params, (String) params.get(AliPayConst.SIGN)) && verifySource((String) params.get("notify_id"));
    ...
    public boolean signVerify(Map<String, Object> params, String sign) {
        ...
        if (params instanceof JSONObject) {
            for (Map.Entry<String, Object> entry : params.entrySet()) {
                if (AliPayConst.SIGN.equals(entry.getKey()) || AliPayConst.ALIPAY_CERT_SN_FIELD.equals(entry.getKey())) {
                    continue;
                }
                TreeMap<String, Object> response = new TreeMap((Map<String, Object>) entry.getValue());
                LinkedHashMap<Object, Object> linkedHashMap = new LinkedHashMap<>();
                linkedHashMap.put(AliPayConst.CODE, response.remove(AliPayConst.CODE));
    ...
    protected Map<String, Object> setSign(Map<String, Object> parameters) {
        ...
        String sign = createSign(SignUtils.parameterText(parameters, "&", AliPayConst.SIGN), payConfigStorage.getInputCharset());

        parameters.put(AliPayConst.SIGN, sign);
        return parameters;
    }
    ...
    private void setNotifyUrl(Map<String, Object> orderInfo, PayOrder order) {
        orderInfo.put(AliPayConst.NOTIFY_URL, payConfigStorage.getNotifyUrl());
        setParameters(orderInfo, AliPayConst.NOTIFY_URL, order);
    }

    private void setReturnUrl(Map<String, Object> orderInfo, PayOrder order) {
        orderInfo.put(AliPayConst.RETURN_URL, payConfigStorage.getReturnUrl());
        setParameters(orderInfo, AliPayConst.RETURN_URL, order);
    }
    ...
    protected Map<String, Object> getOrder(PayOrder order) {
        ...
        switch ((AliTransactionType) order.getTransactionType()) {
            case PAGE:
                bizContent.put(AliPayConst.PASSBACK_PARAMS, order.getAddition());
                bizContent.put(AliPayConst.PRODUCT_CODE, "FAST_INSTANT_TRADE_PAY");
                setReturnUrl(orderInfo, order);
                break;
            case WAP:
                bizContent.put(AliPayConst.PASSBACK_PARAMS, order.getAddition());
                bizContent.put(AliPayConst.PRODUCT_CODE, "QUICK_WAP_PAY");
                setReturnUrl(orderInfo, order);
                break;
            case APP:
                bizContent.put(AliPayConst.PASSBACK_PARAMS, order.getAddition());
                bizContent.put(AliPayConst.PRODUCT_CODE, "QUICK_MSECURITY_PAY");
                break;
            case MINAPP:
                bizContent.put("extend_params", order.getAddition());
                bizContent.put("buyer_id", order.getOpenid());
                bizContent.put(AliPayConst.PRODUCT_CODE, "FACE_TO_FACE_PAYMENT");
                break;
            case BAR_CODE:
            case WAVE_CODE:
            case SECURITY_CODE:
        ...
        orderInfo.put(AliPayConst.BIZ_CONTENT, JSON.toJSONString(bizContent));
    ...
    public String buildRequest(Map<String, Object> orderInfo, MethodType method) {
        ...
        String bizContent = (String) orderInfo.remove(AliPayConst.BIZ_CONTENT);
    ...
    public String getQrPay(PayOrder order) {
        ...
        if (AliPayConst.SUCCESS_CODE.equals(response.getString(AliPayConst.CODE))) {
            return response.getString("qr_code");
        }
        throw new PayErrorException(new PayException(response.getString(AliPayConst.CODE), response.getString("msg"), result.toJSONString()));
    ...
    public Map<String, Object> settle(OrderSettle order) {
        ...
        bizContent.putAll(order.getAttrs());
        parameters.put(AliPayConst.BIZ_CONTENT, JSON.toJSONString(bizContent));
        //设置签名
        setSign(parameters);
    ...
    protected void setAppAuthToken(Map<String, Object> parameters, Map<String, Object> attrs) {
        ...
        setParameters(parameters, AliPayConst.APP_AUTH_TOKEN, (String) attrs.remove(AliPayConst.APP_AUTH_TOKEN));
    }
    ...
    protected void setAppAuthToken(Map<String, Object> parameters) {
        setParameters(parameters, AliPayConst.APP_AUTH_TOKEN, payConfigStorage.getAppAuthToken());
    }
    ...
    public Map<String, Object> downloadBill(Date billDate, BillType billType) {
        ...
        parameters.put(AliPayConst.BIZ_CONTENT, bizContentStr);
        //设置签名
        setSign(parameters);
    ...
    public Map<String, Object> secondaryInterface(Object tradeNoOrBillDate, String outTradeNoBillType, TransactionType transactionType) {
        ...
        //设置请求参数的集合
        parameters.put(AliPayConst.BIZ_CONTENT, getContentToJson((String) tradeNoOrBillDate, outTradeNoBillType));
        //设置签名
        setSign(parameters);
    ...
    private Map<String, Object> setPayeeInfo(Map<String, Object> bizContent, Order order) {
        final Object attr = order.getAttr(AliPayConst.PAYEE_INFO);

        if (attr instanceof String) {
            bizContent.put(AliPayConst.PAYEE_INFO, attr);
        }
        ...
        if (attr instanceof Map) {
            ...
            bizContent.put(AliPayConst.PAYEE_INFO, payeeInfo);
        }
        return bizContent;
    }
